...
void SetDisplayMode(int modeToDisplay) {
		...
		case GAME_SCREEN: glClearColor(0.00117, 0.03137, 0.1490, 0);break;
		case MENU_SCREEN : glClearColor((0.337),(0.26),(0.99),0);break;
	}
}
void initializeEnemyArray() {
	...
	xEnemy[0] = -(200*MAX_STONES)-600;             //START LINE for stone appearance

	for(int i = 0;i<MAX_STONES ;i++) {				//ramdom appearance yIndex for each stone
		yEnemy[i]=rand()%600;
		if(int(yEnemy[i])%2)
			yEnemy[i]*=-1;
		xEnemy[i+1] = xEnemy[i] + 200;				//xIndex of stone aligned with 200 units gap
	}
}
...
void EnemyStoneGenerate(){
		...
		if(xEnemy[0]>=1200){      //If the last screen hits the end of screen then go to Nxt lvl
			GameLvl++;
...
void hartGenerate(){
		...
		if(xHart[0]>=1200){      //If the last screen hits the end of screen then go to Nxt lvl
	...
	for(int i=0; i<MAX_HART ;i++){
		index = i;
        //printf("enter hartGenerate coluction detaction for\n");
...
void display() {
	...
	if(startGame && !gameOver)//startGame =false  gameOver=true
		GameScreenDisplay();
...
void keys(unsigned char key, int x, int y)
{
	...
	if(key == '1')
    {
        ...
        temp_stoneTranslationSpeed = stoneTranslationSpeed;
        stoneTranslationSpeed=0;
    }
    if(key == '2')
    {
        stoneTranslationSpeed = temp_stoneTranslationSpeed;
    }
